...
def compute_attribution(method: str, model, test_loader, args, device: str = 'mps', verbose: bool = True):
    if method == 'wavelet':
        masks, scores = compute_wavelet_attribution(model, test_loader, filterbank_params = {'wavelet': args.wavelet, 'w_len': args.len_w, 'level': args.level, 'fs': args.sample_freq}, device = device, verbose=verbose)
